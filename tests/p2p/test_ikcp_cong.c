...
int main(void)
{
    ...
    /*
     * Early Retransmit, RFC 5827. A segment collects one fastack per later
     * segment acknowledged, so with two outstanding a loss can never produce
     * the two the threshold asks for and the repair falls through to an RTO.
     * Below four outstanding the bar has to come down; at or above it, it must
     * not, or a single reordered ack would retransmit the whole window.
     */
    {
        ikcpcb  *kcp = ikcp_create(1, NULL);
        struct IKCPSEG *first;
        char     payload[1000];
        int      before, after, i;
        char     buf[128];
        ...
        ikcp_wndsize(kcp, 1024, 1024);
        ikcp_setmtu(kcp, 1400);
        ikcp_nodelay(kcp, 1, 10, 2, 0); /* fast retransmit asks for 2 acks */
        kcp->rmt_wnd = 1024;
        kcp->cwnd    = 1024;
        kcp->updated = 1;
        kcp->current = 1000;
        ...
        ikcp_send(kcp, payload, (int)sizeof(payload));
        ikcp_send(kcp, payload, (int)sizeof(payload));
        ikcp_flush(kcp); /* both on the wire: two outstanding */

        first          = iqueue_entry(kcp->snd_buf.next, struct IKCPSEG, node);
        before         = (int)first->xmit;
        first->fastack = 1; /* the one behind it was acked, this one was not */
        kcp->current   = 1010; /* nowhere near the RTO */
        ikcp_flush(kcp);
        after = (int)first->xmit;

        snprintf(buf, sizeof(buf), "xmit %d -> %d on 1 ack, 2 outstanding", before, after);
        check("one ack repairs a small window", after > before, buf);
        ikcp_release(kcp);

        /* Same single ack, but with a window big enough to earn two. */
        kcp = ikcp_create(1, NULL);
        ikcp_setoutput(kcp, counting_output);
        ikcp_wndsize(kcp, 1024, 1024);
        ikcp_setmtu(kcp, 1400);
        ...
        snprintf(buf, sizeof(buf), "xmit %d -> %d on 1 ack, 6 outstanding", before, after);
        check("one ack is not enough with room", after == before, buf);
        ikcp_release(kcp);
    }
